...
export class MatchingService {
    ...
    async addMatchRequest(request: MatchingRequestDto, user: User): Promise<void> {
        ...
        const cancellationKey = `${key}-${name}`;
        ...
        if (role === "driver") {
            ...
            if (!queue.some((driver) => driver.username === name)) {
                queue.push(request); // 운전자를 큐에 추가
                console.log(`운전자 ${name}이 ${key} 큐에 추가되었습니다.`);

            } else {
                console.log(`운전자 ${name}의 요청이 이미 존재합니다.`);
            }
            ...
            if (!queue.some((passenger) => passenger.username === name)) {
                queue.push(request); // 탑승자를 큐에 추가
                console.log(`탑승자 ${name}이 ${key} 큐에 추가되었습니다.`);

            } else {
                console.log(`탑승자 ${name}의 요청이 이미 존재합니다.`);
            }
        }
    ...
    async cancelMatching(key: string, user: User): Promise<void> {
    ...
    // 매칭 진행 상황 확인 메서드
    async getMatchingStatus(user: User, key: string): Promise<{ status: MatchingStatus; rideRequestId: number }> {
        if (user.role === 'driver') {
            // 운전자인 경우
            if (this.matchingRooms.has(user.name)) {
                // return MatchingStatus.Matched; // 매칭 성공
                const room = this.matchingRooms.get(user.name)!;
                return { status: MatchingStatus.Matched, rideRequestId: room.rideRequestId }; // rideRequestId 반환
            } else if (this.driverQueues.get(key)?.some((driver) => driver.username === user.name)) {
                return { status: MatchingStatus.Waiting, rideRequestId: -1 };
            } else if (this.processingDrivers.has(user.name)) {
                return { status: MatchingStatus.Waiting, rideRequestId: -1 };
            } else {
            ...
            const matchedRoom = Array.from(this.matchingRooms.values()).find((room) =>
                room.passengers.some((passenger) => passenger.username === user.name)
            );

            if (matchedRoom) {
                return { status: MatchingStatus.Matched, rideRequestId: matchedRoom.rideRequestId }; // rideRequestId 반환
            } else if (this.passengerQueues.get(key)?.some((passenger) => passenger.username === user.name)) {
                return { status: MatchingStatus.Waiting, rideRequestId: -1 };
            } else {
    ...
    // 개인 나가기 요청 처리 메서드
    async leaveMatch(user: User, rideRequestId: number): Promise<void> {
        if (user.role === 'driver') {
            ...
            if (rideRequest) {
                ...
                console.log(`운전자 ${user.name}의 매칭이 취소되고 모든 관련 데이터가 삭제되었습니다.`);
            } else {
                console.log(`운전자 ${user.name}의 매칭 정보를 찾을 수 없습니다.`);
            ...
            if (ridePassenger) {
                ...
                console.log(`탑승자 ${user.name}의 매칭이 취소되었습니다.`);
            } else {
                console.log(`탑승자 ${user.name}의 매칭 정보를 찾을 수 없습니다.`);
                throw new UnauthorizedException('해당 매칭에 대한 권한이 없습니다.');
            }
        }
    }
    ...
    // 탑승자 강퇴 메서드
    async kickPassenger(driver: User, passengerId: number): Promise<void> {
        ...
        console.log(`운전자 ${driver.name}이 탑승자 ${passenger.name}을 강퇴하였습니다.`);
    }
